...
pub async fn main() {
  ...
  let app = App::new(crate_name!())
    ...
    .subcommand(SubCommand::with_name("new")
      .about("Deploys your repository to a new app with a deploy profiles in anycloud.json")
      .arg_from_usage("-p --deploy-profile=[DEPLOY_PROFILE] 'Specifies the name of the deploy profile from anycloud.json if multiple'")
    ...
    .subcommand(SubCommand::with_name("info")
      .about("Displays all the apps deployed with the deploy profiles in anycloud.json")
    )
    .subcommand(SubCommand::with_name("terminate")
      .about("Terminate an app with the provided ID hosted in one of the deploy profiles in anycloud.json")
      .arg_from_usage("<APP_ID> 'Specifies the alan app to terminate'")
    )
    .subcommand(SubCommand::with_name("upgrade")
      .about("Deploys your repository to an existing app hosted in one of the deploy profiles in anycloud.json")
